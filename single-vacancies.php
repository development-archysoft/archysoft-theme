    ...
    <div class="single_ps">
        <div class="container">
            <div class="post_left">
                ...
                <img class="banner_posts__img vacn_img_banner" decoding="async" src="<?php echo the_post_thumbnail_url('full') ?>" srcset="<?php echo the_post_thumbnail_url('medium'); ?> 1920w , <?php echo the_post_thumbnail_url('thumbnail'); ?> 1199w" alt="<?php echo get_post_meta(get_post_thumbnail_id(), '_wp_attachment_image_alt', true); ?>">
...
<div class="leave_feedback">
    <div class="container">
        <div class="leave_feedback_call">CONTACT US</div>
        <div class="leave_feedback_title">If you are interested in this position or you have some questions, please fill in the contact form</div>
        <form enctype="multipart/form-data" method="POST" class="leave_resume">
            <input type="hidden" name="form_id" value="leave_resume_form">
            <label>
                <p>First Name</p>
            ...
            <label class="resume">
                ...
                <input name="resume" title="Choose a file" type="file" required="" accept=".pdf,.doc,.docx">
            </label>
            <div class="g-recaptcha" data-sitekey="6LcMvPwpAAAAAMr81q3RZy0AkDtJHFL6LCJw8ZFp"></div>
            <input class="leave_resume_submit btn" type="submit" value="Submit">
        </form>
    </div>
</div>
<div class="resume_send_ok">
    Thank you for your message. It has been sent.
</div>
</div>
<script>
    let leave_resume_form = document.querySelector('.leave_resume');
    let resume_send_ok = document.querySelector('.resume_send_ok');

    leave_resume_form.addEventListener('submit', function(e) {
        e.preventDefault(); // Попереджаємо типову подію відправки форми

        let formData = new FormData(this);

        // Видаляємо значення інпуту файлу, якщо воно не вибране
        ...
        .then(response => {
            if (response.ok) {
                ...
                dataLayer.push({
                    'event': 'formSubmitVacancy',
                    'formType': 'leave_resume_form'
                });
                console.log('dataLayer.push event sent');

                this.reset();
                resume_send_ok.classList.add('comment_send_ok_active');
                setTimeout(() => {
                    resume_send_ok.classList.remove('comment_send_ok_active');
                }, 4000);
            }
        })
